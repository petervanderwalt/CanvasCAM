...
function tabDepth(toolpath, contour, along, cutDepth) {
  ...
  const tabs = (toolpath.tabs || []).filter((tab) => tab.contourIndex === contourIndex);
  ...
  for (let index = 1; index < contour.length; index += 1) {
    ...
    for (let sample = 0; sample <= count; sample += 1) {
      ...
      target.push({
        x: a.x + (b.x - a.x) * ratio,
        y: a.y + (b.y - a.y) * ratio,
        z: tabDepth(toolpath, contour, travelled + length * ratio, depth),
        ...metadata,
      });
    }
    travelled += length;
...
async function build(version, toolpaths) {
  ...
  const maxDiameter = Math.max(1, ...toolpaths.map((path) => Number(path.toolDiameter) || 1));
  ...
  const minCutter = Math.max(0.2, Math.min(...toolpaths.map((path) => Number(path.toolDiameter) || 1)));
  const cell = Math.max(0.1, Math.min(minCutter * 0.16, Math.max(width, height) / 960));
  ...
  for (const toolpath of toolpaths) {
    ...
    const metadata = {
      cutter,
      trochoidRadius: toolpath.trochoidEnabled ? Math.max(0, Number(toolpath.trochoidRadius) || 0) : 0,
      ...
      vbit: toolpath.operation === "vcarve" || toolpath.operation === "v-carve" || toolpath.operation === "chamfer",
      angle: clamp(Number(toolpath.cutterAngle) || 90, 10, 170),
    };
    if (toolpath.motionPaths?.length) {
      for (const path of toolpath.motionPaths) addMotionSamples(samples, path.points || [], step, metadata);
    } else {
      for (const depth of toolpath.passDepths || []) {
        for (const contour of toolpath.previewContours || []) addContourSamples(samples, toolpath, contour, depth, step, metadata);
      }
    }
  }
  if (version !== latestBuildVersion) return;
  self.postMessage({ type: "progress", version, progress: 0, total: samples.length });
  ...
  for (let index = 0; index < samples.length; index += 1) {
    ...
    if (index % 750 === 0) {
      if (version !== latestBuildVersion) return;
      self.postMessage({ type: "progress", version, progress: Math.round(index / Math.max(1, samples.length) * 100), total: samples.length });
      await yieldWorker();
    }
  }
  if (version !== latestBuildVersion) return;
  ...
  for (let index = 0; index < samples.length; index += 1) {
    const sample = samples[index], offset = index * 6;
...
self.addEventListener("message", ({ data }) => {
  ...
  build(data.version, data.toolpaths || []).catch((error) => {
    if (data.version === latestBuildVersion) self.postMessage({ type: "error", version: data.version, message: error?.message || "3D simulation failed." });
  });
});
